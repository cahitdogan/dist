...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (window.AutoSave) {
        new AutoSave('article-form', 'edit-article-autosave-' + articleId, 30000);
    }
});
...
async function loadArticle() {
    try {
        ...
        // Get all articles and find the one we want
        ...
        slugInput.value = currentArticle.slug;
        excerptInput.value = currentArticle.excerpt || '';
        contentInput.value = currentArticle.content;
        categorySelect.value = currentArticle.category_id || '';
        featuredImageInput.value = currentArticle.featured_image_url || '';
        readTimeInput.value = currentArticle.read_time || 5;
        ...
        if (currentArticle.featured_image_url) {
            previewImg.src = `http://localhost:3000${currentArticle.featured_image_url}`;
        ...
        updateWordCount(currentArticle.content);
    } catch (error) {
...
async function handleSubmit(event) {
    ...
    try {
        ...
        const result = await api.updateArticle(articleId, articleData);
